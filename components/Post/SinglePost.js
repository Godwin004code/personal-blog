...
const SinglePost = ({img, title, content, category, author, date, time, excerpt}) => {
  ...
  <Head>
    ...
    <div id='long_link' className={_styles.long_link}>
      <Link href='/'><a>Home <span>&raquo;</span></a></Link> <Link href={`/category/${category}`}><a>{category} <span>&raquo;</span></a></Link> <a>{title}</a>
    </div>
